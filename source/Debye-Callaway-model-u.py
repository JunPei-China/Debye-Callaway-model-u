...
def measure_data():
    LogitudinalSoundVelocity = float(input("请输入纵波声速(单位:m/s)。 %s"%prompt))
    TransverseSoundVelocity = float(input("请输入横波声速(单位:m/s)。%s"%prompt))
    Density = float(input("请输入样品的密度(单位:g/cm^-3)。 %s"%prompt))
    NumberAtoms = float(input("请输入单胞中原子的个数。 %s"%prompt))
    VolumeCell = float(input("请输入单胞的体积(单位:A^-3)。 %s"%prompt))
...
def intrinsic_data():
    ...
    for x in range(NumberAtom):
        ...
        for y in range(NumberElement):    
            ...
            # 离子半径更合理
            AtomicRadius = float(input("请输入%s的离子半径，单位(pm)。 %s"%(NameElements,prompt)))
        ...
        DopingOrNot = str(input("该位置是否有掺杂？y/n %s"%prompt))
        Doping = []
        if DopingOrNot[0].lower() == "y":
            NameDoping = str(input("请输入第%d个位置掺杂元素的元素符号。 %s"%(x+1,prompt)))
            DopingSubscript = float(input("请输入第%d个位置掺杂元素的下标量。 %s"%(x+1,prompt))) 
            DopingMass = PeriodicTable[NameDoping]["RelativeAtomicMass"]
            # 原子半径不合适
            #DopingRadius = PeriodicTable[NameDoping]["AtomicRadius"]["value"]
            # 修改为离子半径
            DopingRadius = float(input("请输入%s的离子半径。 单位(pm) %s"%(NameDoping,prompt)))
...
def test():
    ...
    if input("需要设置应力波动调整参数varepsilon的值吗？(y/n) %s"%prompt)[0].lower() == "y":
        Varepsilon = float(input("请输入应力波动调整参数varepsilon。 %s"%prompt))
    else:
